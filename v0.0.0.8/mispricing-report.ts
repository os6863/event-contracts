/**
 * mispricing-report.ts — v0.0.0.8
 *
 * Same as v0.0.0.7 (ensemble signal, signal history, live GitHub Pages
 * report), plus: history entries now carry `marketId`, and the report
 * renders a "Track record" section from real settlement data — filled
 * in by the companion script `check-outcomes.ts`, which checks each
 * logged market's actual on-chain `winningOutcome` once it closes and
 * scores both DreamDEX's own price and our ensemble with a Brier score
 * (see check-outcomes.ts's own header for why Brier, not just
 * right/wrong side).
 *
 * This script alone still makes NO accuracy claim — it only logs
 * predictions. The claim only exists once `check-outcomes.ts` has been
 * run against markets that have genuinely settled.
 *
