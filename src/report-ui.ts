...
import { validMarketId, computeSimulatedEdge, computeUniqueMarketBrier, PRICE_SOURCE_QUALITY_LABEL, type SimulatedEdgeBucket } from "./analysis.js";
...
const signal = (v: unknown): SignalAgreement => v === "strong" || v === "conflicted" || v === "weak" ? v : "none";
const label = (v: unknown) => ({ strong: "Strong signal", conflicted: "Conflicting signal", weak: "Weak signal", none: "No signal" })[signal(v)];
...
function explanation(r: ReportRow): string {
  ...
  if (r.agreement === "conflicted") return "Both estimates differ from DreamDEX by at least 15 points, but in opposite directions — the two estimators disagree with each other, not just with the market. Treated as model disagreement, not a directional signal, and excluded from the simulated-edge calculation.";
  if (r.agreement === "weak") return "One estimate differs by 15 points and the other does not — a single estimator flagging alone, not two-estimator agreement.";
...
function renderCard(r: ReportRow, index: number): string {
  ...
  return `<article class="signal-card" data-signal="${state}">
    ...
    ${[["Opening price", usd(r.openingPrice)], ["Agent price", `${usd(r.currentPrice)}${r.priceSources?.length ? ` <small>(${PRICE_SOURCE_QUALITY_LABEL[r.priceSourceQuality ?? "single-source-fallback"]} — ${escapeHtml(r.priceSources.join(", "))})</small>` : ""}`], ["Price move", move(r.movePct)], ["Order book", escapeHtml(orderBookLabel(r))], ["Naive baseline", `${formatProbability(r.naiveEst)}${finite(r.realizedVolAnnual) ? ` <small>(${(r.realizedVolAnnual! * 100).toFixed(0)}% realized vol, 7d)</small>` : r.realizedVolAnnual === null ? ` <small>(fixed volatility assumption — realized vol unavailable)</small>` : ""}`], ["LLM estimate", formatProbability(r.llmEst)], ["Time left at observation", finite(r.minutesLeft) ? `${r.minutesLeft.toFixed(1)} min` : "n/a"]].map(([k,v]) => `<div><dt>${k}</dt><dd>${v}</dd></div>`).join("")}</dl>
...
export function renderReport(rows: ReportRow[], history: HistoryEntry[], generatedAt: string): string {
  ...
  const rank = { strong: 0, conflicted: 1, weak: 2, none: 3 };
  ...
  const conflicted = valid.filter(r => r.agreement === "conflicted").length;
...
<div class="snapshot-bar"><span>SNAPSHOT REPORT · v0.1.0</span><span>Generated <time datetime="${escapeHtml(generatedAt)}">${time(generatedAt)}</time></span></div><p class="muted" style="margin-top:-10px;margin-bottom:22px">These are recorded observations, not streaming quotes. Market prices and trading status may have changed since this report was generated.</p>
<div class="kpis"><div class="kpi"><span>Markets in this run</span><strong>${rows.length.toString().padStart(2,"0")}</strong><small>${valid.length} usable comparisons</small></div><div class="kpi"><span>Strong signals</span><strong class="green">${String(strong).padStart(2,"0")}</strong><small>Both estimators clear the gate</small></div><div class="kpi"><span>Conflicting</span><strong class="negative">${String(conflicted).padStart(2,"0")}</strong><small>Estimators disagree with each other</small></div><div class="kpi"><span>Weak signals</span><strong class="yellow">${String(weak).padStart(2,"0")}</strong><small>One estimator flagged alone</small></div><div class="kpi"><span>Trading mode</span><strong style="font-size:25px">Read-only</strong><small>No orders · agent computation uses testnet transactions</small></div></div>
${valid.length > 0 && strong === 0 && conflicted === 0 && weak === 0 ? `<p class="section-intro" style="margin-top:-40px;margin-bottom:56px">All ${valid.length} usable markets in this run show DreamDEX and the independent ensemble agreeing within the 15-point threshold. That's an expected outcome, not a malfunction — a signal means the estimators found a real divergence, and most of the time there isn't one to find.</p>` : ""}
<section id="signals"><div class="section-heading"><div><span class="eyebrow">THE INDEPENDENT VIEW</span><h2>Opportunity board</h2></div><div class="filters" role="group" aria-label="Filter signals">${[["all","All"],["strong","Strong"],["conflicted","Conflicting"],["weak","Weak"],["none","No signal"]].map(([v,l])=>`<button type="button" data-filter="${v}" aria-pressed="${v === "all"}">${l}</button>`).join("")}</div></div><p class="section-intro">Ranked by signal class, then divergence. Strong requires both estimators to differ from DreamDEX by at least 15 points in the same direction. Conflicting means both cleared that threshold but disagree with each other about which direction — that is model disagreement, not a directional signal.</p><div class="board">${sorted.map(renderCard).join("")}</div>${!rows.length ? '<div class="empty">No active markets with sufficient time remaining were found in this run.</div>' : ""}<div id="filter-empty" class="empty" hidden>No markets match this filter.</div><p id="board-count" class="board-count" aria-live="polite">Showing ${rows.length} of ${rows.length} markets</p><noscript><p class="muted">All markets are shown. Enable JavaScript to filter them.</p></noscript></section>
...
<section><span class="eyebrow">BUILT THROUGH REAL TESTNET RUNS</span><h2>Reliability comes from iteration.</h2><p class="section-intro">Each release records the problems found in live testing and the fixes that followed.</p><div class="timeline"><div><b>v0.0.0.3</b><h3>The correct price foundation</h3><p>Replaced the unusable strike field with DreamDEX's actual opening-price source.</p></div><div><b>v0.0.0.7–9</b><h3>A second model checks the signal</h3><p>Added ensemble agreement, real settlement scoring and a time-aware probability baseline.</p></div><div><b>v0.0.0.10</b><h3>Check the response, expose the evidence</h3><p>Validate final answers, retain receipt references and distinguish expired or unavailable estimates.</p></div><div><b>v0.1.0</b><h3>Honest labels, measured inputs</h3><p>Realized volatility replaces a fixed assumption where available; conflicting-direction signals get their own class instead of being folded into "weak"; price-source quality is labeled for what it actually is.</p></div></div></section>
